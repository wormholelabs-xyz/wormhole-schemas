...
// Embedded schemas generated by build.rs.
include!(concat!(env!("OUT_DIR"), "/builtin_schemas.rs"));
...
impl Registry {
    /// Create a registry from the compiled-in schemas only.
    pub fn builtin() -> Result<Self> {
        let mut schemas = HashMap::new();
        for (key, json_str) in BUILTIN_SCHEMAS {
            let value: serde_json::Value = serde_json::from_str(json_str)
                .with_context(|| format!("parsing builtin schema {}", key))?;
            let s = schema::parse_schema(value)
                .with_context(|| format!("invalid builtin schema: {}", key))?;
            schemas.insert(key.to_string(), s);
        }
    ...
    /// Load from compiled-in schemas, then layer on overrides from a directory.
    pub fn builtin_with_overrides(schema_dir: &Path) -> Result<Self> {
        let mut schemas = HashMap::new();
        for (key, json_str) in BUILTIN_SCHEMAS {
            let value: serde_json::Value = serde_json::from_str(json_str)
                .with_context(|| format!("parsing builtin schema {}", key))?;
        ...
        // Directory schemas override built-ins
